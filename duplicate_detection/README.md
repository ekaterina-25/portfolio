...
- **`spare_parts_system2.xlsx`** — 165 items, simulates a second ERP system with
  overlapping products formatted differently (for cross-system comparison use cases)
...
Running on the screw dataset (`Standard` column as discriminator):

```
Loaded 2 300 rows
Discriminator column: 'Standard'
...
Step 4 — Discriminator filter (Standard)
  Removed 1 414 pairs with conflicting standards (e.g. DIN 931 vs DIN 933)
  Verified pairs after filter: 879

Groups found: 166   |   Rows flagged: 524 / 2 300 (23 %)
```

Sample duplicate group found (100 % similarity):

| item_id | similarity_pct | specification | Standard |
|---|---|---|---|
| 2001144 | 100 % | 7/16 UNC x 1 COATED | ANSI B18.3-8.8 ZNE |
| 2001140 | 100 % | 7/16 UNC x 1 COATED | ANSI B18.3-8.8 ZNE |

Same product entered twice. Simple Excel deduplication would not catch this because
the values are not byte-for-byte identical in all fields.
